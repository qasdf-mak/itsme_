					...
					<view class="width-full height pt15 pb15 border-box">
						...
						<view class="width-full" v-if="perfectIndex === 4">
							<view class="width-full mt30 flex flex-col flex-center align-items-center">
								<view class="perfect-avator-box" @click="chooseAvatar">
									<view class="width-full height" v-if="avatorImg">
										<image :src="avatorImg | imageFilter" class="width-full height" mode="widthFix">
										</image>
									</view>
		...
		methods: {
			...
			nextStep() {
				...
				if (this.perfectIndex === 3) {
					if (this.coverImg && this.profileData.birthday.content && this.profileData.height.content && this
				...
				faceFind().then(
					img => {
						...
						UserService.baiduFace({
							...
							// this.faceSuccess()
							if (res.code === 1 && res.data && res.data.result && res.data.result.face_num ===
								1) {
							...
							pathToBase64(img.tempFilePath)
								.then(base64 => {
									UserService.baiduFaceBd({
										image1: this.faceImg,
										image2: base64.substring(base64.indexOf(';base64,') + 8,
											base64.length)
									}).then(res => {
										// this.toUpFaceImg()
										if (res.code === 1 && res.data && res.data.result && res
											.data.result.score >= 80) {
											this.toUpFaceImg()
										} else {
											uni.hideLoading()
											this.unifyType = 12
