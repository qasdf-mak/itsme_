	...
	export default {
		...
		methods: {
			...
			getLabel() {
				UserService.getuserexpect({
					...
					if (res.code === 1) {
						...
						let isExpectItem = []
						isExpectItem = this.expectData[0].list.filter(item => item.is_choice === 1)
						this.expectList = isExpectItem.map(item => item.id)
						let profession = this.expectData[1].list.filter(item => item.is_choice === 1)[
							0].id
			...
			submitExpect() {
				let expectid0 = []
				let expectid1 = []
				let expect1 = this.expectData[0].list.filter(item => {
					if (item.is_choice == 1) {
					return expectid0.push(item.id) 
					}
				})
				let expect2 = this.expectData[1].list.filter(item => {
					if (item.is_choice == 1) {
					return expectid1.push(item.id) 
					}
				})
				let expect = expectid0.concat(expectid1)
