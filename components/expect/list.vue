<template>
	<view class="width-full flex flex-col lh1" :class="type === 'page' ? 'expect-box-1' : 'expect-box-2'">
		...
		<view class="width-full flex flex-col align-items-center flex-center relative">
			<view class="fs16 color-deep fw600 pt10">
				{{step === 1 ? '你希望遇到怎样的人' : '你是怎样的人'}}
			</view>
			<view class="pt10 fs13 color-999999">
				{{step === 1 ? '去人海茫茫中找到灵魂共鸣的Ta吧' : '选择你的个性标签，平台将为您匹配更合适的人'}}
			</view>
			<view class="top-line-box mt10 mb10"></view>
		...
		<view class="width-full flex-1 float-hidden">
			<scroll-view scroll-y="true" class="width-full" :scroll-top="scrollTop" :class="type === 'page' ? 'scroll-box-1' : 'scroll-box-2'">
						...
						<view class="width-full" v-if="item.type === 1 && item.list && item.list.length > 0 && item.type_info">
							<view class="ml20 mr20">
									<view class="width-full flex align-items-center flex-between">
										...
										<view class="flex align-items-center" v-if="item.list && item.list.length > 7" @click="showMore(index)">
								...
								<view class="ml35 flex flex-wrap mt10 mb10">
									<view v-for="(lable, lindex) in item.list" :key="lindex">
										<view
											class="lable-box fs12 border-box text-center mr12 mb8"
											:class="lable.is_choice === 1 ? 'active' : 'default'"
											v-show="(lindex < 7 && !item.isShow) || item.isShow"
									...
									<view
										class="lable-box border-box mr12 mb8 default flex align-items-center flex-center"
										@click="addLable(index)"
									>
										<view class="icon_font mr5 fs13">
											&#xe62c;
										</view>
										<view class="fs12">新增</view>
									</view>
								</view>
							</view>
		...
		<view class="width-full relative">
			<view class="pt10 pb15 flex align-items-center flex-center">
				<view class="bottom-btn-box bg-2 text-center fs14 color-white" @click="confirmBable">
					{{step === 1 ? '下一步' : '完成'}}
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			toProfile() {
				if (this.type === 'page') {
					uni.redirectTo({
						url: '/pages/my/profile?isMypass=' + this.isMypass
					})
				} else {
