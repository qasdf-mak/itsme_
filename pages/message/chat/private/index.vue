<template>
	<view class="width-full height100vh lh1 bg-itsme-1">
			...
			<view class="fixed top left right bg-itsme-1">
				...
				<view class="width-full top-nav-box flex">
					<view class="top-nav-item height flex align-items-center flex-left ml15">
						<view class="icon_font fs25 color-white" @click="goList">
							&#xe60e;
						</view>
					</view>
					<view class="flex-1 height flex align-items-center flex-center">
						<view class="fs18 color-white fw600 nowrap1">
							{{friendInfo && friendInfo.nickname || ''}}
						</view>
						<image src="../../../../static/itsme/its_7.png" class="top-nav-img ml6" mode="widthFix"
							v-if="friendInfo && friendInfo.is_vip && friendInfo.is_vip === 1"></image>
						<image src="../../../../static/itsme/its_2.png" class="top-nav-img ml6" mode="widthFix"
		...
		<view class="pl15 pt15 pr15 pb15">
			...
			<view class="width-full" @click="closeMenu">
				<view class="width-full pt10 pb15">
					<u-divider bg-color="">{{allHistoryLoaded ? '已经没有更多的历史消息' : '下拉获取历史消息'}}</u-divider>
				</view>
				<view class="width-full">
				...
				<view class="fillBox" v-if="closeMenuPop"></view>
				<view class="fillBox1" v-if="closeEmojiPop"></view>
				<view class="width-full">
		...
		<!-- main begin -->
		<!-- 聊天按钮 -->
		<send-box :friend="friendUser" :selfTask="selfTask" @competence="competence" :closeMenuPop="closeMenuPop"
			@closeEmoji="omomomo" @manNoVipPop="manNoVipPop" @sendSuccess="sendSuccess" @sendGift="sendGift"
			@showMore="showMore" @omomomo="omomomo" @sendVoucher="sendVoucher" @sendPosition="sendPosition"
			@showCloseEmoji="showCloseEmoji"></send-box>
		<!-- 聊天按钮 -->
		<!-- 送礼物 -->
		...
		<!-- 权限弹窗 -->
		<!-- 充值vip弹框 -->
		<!-- @paysuccess="updateInformationPop" -->
		<view v-if="showMemberPay">
			<member-pay :showMemberPay="showMemberPay" :fromPageId="friendId" fromPage="chat"
				@closePop="showMemberPay = false" @closepopsuccess="closePopSuc" :powerData="powerData"
				@paysuccesss="updateInformationPop" :centent="memberCentent">
			</member-pay>
		</view>
		<!-- 充值vip弹框 -->
		<!-- 完善资料 -->
	...
	export default {
		...
		methods: {
				...
				this.loginExpire().then(res => {
					if (res) {
						Promise.all([
							...
							// 	let GtSdk = plus.ios.importClass("GeTuiSdk");
							// 	GtSdk.setBadge(2)
							// }
						})
					}
			...
			getChatMsgNum() {
				...
				ChatService.chatnum().then(res => {
					if (res.code === 1) {
						...
						this.claerBadge(res.data.total)
					}
				})
			},
			...
			updateInformationPop() {
				console.log('11111111111111111111111111111111111111112')
			},
			omomomo(index) {
			...
			comeMessage(friendId, message) {
				if (friendId == this.friendId) {
					// this.getPower()
					this.geiNewChat('receiver', message)
				}
			},
			showCloseEmoji(state) {
				if (state) { // state为 true 时 打开表情 为 false 时 关闭表情
					this.closeEmojiPop = true
				} else {
					this.closeEmojiPop = false
				}
			},
			showMore(state) {
			...
			sendPosition() {
				...
				uni.chooseLocation({
					success: function(res) {
						...
						that.position.addr = res.name
						that.sendImMessage('map', that.friendUser, that.position).then(
							(res) => {
								console.log('发送成功')
								console.log(res)
								that.sendSuccess(res.data)
							},
							(err) => {
								console.log('发送失败')
								console.log(err)
			...
			// 	}
			// 	// 女性真人，聊过的人不能再聊提示去女神认证
			// 	if (this.power.msg_type === 'msg' && this.power.msg_type_id === 2006) {
			// 		this.addTipMessage(7)
			// 		this.scrollToBottom()
			...
			// 			}
			// 			// 收8句
			// 			if (this.power && this.power.do === 'suc' && this.power.msg_type_id === 2004 && !
			// 				this
			// 				.isPopTipTwo) {
			...
			// 			}
			// 			// 男性非vip,消息隐藏提示女性
			// 			if (this.power && this.power.do === 'suc' && this.power.msg_type_id === 2005 && !
			// 				this
			// 				.isPopTipOne) {
			...
			sendGift(sex) {
				...
				if (sex === 0) {
					console.log('女')
					this.toJumpPage(1, '../../../my/commission')
				} else {
			...
			useVoucher(payload) {
				if (this.user.is_vip === 1) {
					...
					this.showMemberPay = true
					this.memberCentent = '使用抵用券来购买VIP更省钱哦~'
				}
			...
			// 历史消息记录
			getHistoryMessage() {
				let phoneType = uni.getSystemInfoSync().platform === 'ios' ? 2 : 1
			...
			// 获取新的消息
			geiNewChat(type, message) {
				message.payload = JSON.parse(message.payload)
				this.messageList.push(message)
				this.changeItem(message)
				this.scrollToBottom()
			...
			// 服务端接收消息替换原消息
			changeItem(message) {
				message.payload = this.isJsonString(message.payload)
				this.messageList = this.messageList.map(item => {
